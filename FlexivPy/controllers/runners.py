...
def blocking_runner(robot, controller, timeout=60, callback=None):
    ...
    while True:
        ...
        s = robot.get_robot_state()
        elapsed_time = (
            tic - tic_start if not hasattr(robot, "step") else counter * controller.dt
        )
        ...
        if hasattr(robot, "step"):
            num_steps = (
                int(controller.dt // robot.dt) if controller.dt >= robot.dt else 1
            )
...
class NonBlockingRunner:
    def __init__(self, robot, controller, timeout=60, callback=None):
        s = robot.get_robot_state()
        if hasattr(controller, "setup"):
            controller.setup(s)
        else:
    ...
    def loop(self):
        ...
        while self.running:
            ...
            elapsed_time = (
                tic - tic_start
                if not hasattr(self.robot, "step")
                else counter * self.controller.dt
            )
            ...
            if hasattr(self.robot, "step"):
                num_steps = (
                    int(self.controller.dt // self.robot.dt)
                    if self.controller.dt >= self.robot.dt
                    else 1
                )
                for _ in range(num_steps):
                    self.robot.step()
